...
import { birdsData } from "../../lib/data/birds";
...
export default function BirdsContent() {
  ...
  useEffect(() => {
    if (user) {
      getChecklist(user.uid).then((checklist) => {
        setCaughtStars(checklist.birds || {});
      });
    }
  }, [user]);
  ...
  const handleStarSelect = async (name: string, value: number) => {
    ...
    if (user) {
      ...
      checklist.birds = newCaughtStars;
      await saveChecklist(user.uid, checklist);
    }
  };

  // FILTERING
  let filtered = birdsData.filter(bird => {
    const matchesSearch =
      bird.name.toLowerCase().includes(search.toLowerCase()) ||
      bird.location.toLowerCase().includes(search.toLowerCase());

    const matchesWeather =
      !weatherFilter || bird.weather.includes(weatherFilter);

    const matchesTime =
      !timeFilter || bird.time.includes(timeFilter);
      ...
      <div className="heartopia-table-container">
        <table className="heartopia-table">
          ...
          <tbody>
            {filtered.map(bird => (
              <tr key={bird.name}>
                <td className="heartopia-sticky-col">{bird.name}</td>
                <td>{bird.location}</td>
                <td>{bird.hobbyLevel}</td>
                  ...
                  <div className="flex justify-center gap-3">
                    {[1, 2, 3, 4, 5].map(star => (
                      <div key={star} className="flex flex-col items-center">
                        <button
                          onClick={() => handleStarSelect(bird.name, star)}
                          className={`px-2 py-1 rounded text-sm ${
                            caughtStars[bird.name] === star
